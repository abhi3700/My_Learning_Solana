...
1. **Signatures**: array of signatures to get permissions within a transaction. E.g. sending tokens. Unlike Ethereum, it contains many signers (if present).
2. **Metadata** contains list of accounts that will be referenced/used in this transaction and a “recent” block hash.
...
3. **Instruction**: contains 3 pieces of data:
...
A transaction may contain multiple **instructions**. Once a node has received a transaction, it will parse the instructions, and call related **on-chain programs** according to **program_id** param passed by every instruction.
...
Every Solana account/program has these on-chain resources: **RAM**, **file**, **CPU(Compute budget)**, etc.
...
multiple instructions can be bundled into a **message** called "**transaction**".
...
It represents an account address of _Base58_ format, also in **Instruction** the **ProgramId** is also of the same format.
...
- `data` stores the content of the file which is buffer of binary data. Every file/account is created by program, which called **owner**.
...
### Q. Can the account size be modified later?

Accounts size are fixed at creation time, but can be adjusted using [realloc](https://solanacookbook.com/references/programs.html#accessing-clock-directly-inside-an-instruction)
